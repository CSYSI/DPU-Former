    ...
    try:
        for i, (images, gts, edges) in enumerate(train_loader, start=1):
            ...
            if i % 20 == 0 or i == total_step or i == 1:
                print('{} Epoch [{:03d}/{:03d}], Step [{:04d}/{:04d}], Total_loss: {:.4f} Loss1: {:.4f} Loss2: {:0.4f}, LR [{:.6f}]'.
                      format(datetime.now(), epoch, opt.epoch, i, total_step, loss.data, loss_init.data, loss_final.data, current_lr)) # loss_edge.data
                logging.info(
...
if __name__ == '__main__':
    ...
    parser.add_argument('--trainsize', type=int, default=352, help='training dataset size')#384
    parser.add_argument('--clip', type=float, default=0.5, help='gradient clipping margin')
    parser.add_argument('--decay_rate', type=float, default=0.1, help='decay rate of learning rate')
    parser.add_argument('--decay_epoch', type=int, default=40, help='every n epochs decay learning rate')#80
    ...
    parser.add_argument('--save_path', type=str,default='./DPU_Former/resluts/', help='the path to save model and log')
    ...
    model = torch.nn.DataParallel(Network_UF_v2(channels=128), device_ids=device_ids)
    ...
    for epoch in range(1, opt.epoch):
        ...
        writer.add_scalar('learning_rate', cosine_schedule.get_last_lr()[0], global_step=epoch)
