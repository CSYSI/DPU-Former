...
class Mlp(nn.Module): # gated linear feed-forward network (GLFFN)
...
class Attention(nn.Module):  # Global-local mixed attention (GLMA)
    ...
    def forward(self, x, H, W):
        ...
        x_g = (attn @ v).transpose(1, 2).reshape(B, N, C)
        x = self.fusion(x_g, x_l, H, W)
        x = self.proj(x)
        x = self.proj_drop(x)
...
class GLformer(nn.Module): #DPU-Former encoder
...
class DWConv(nn.Module):
    def __init__(self, dim=768):
        ...
        self.dwconv3 = nn.Conv2d(dim//4, dim//4, 3, 1, 1, bias=True, groups=dim//4)
        self.dwconv3_1 = nn.Conv2d(dim, dim, 3, 1, 1, bias=True, groups=dim)
    def forward(self, x, H, W):
        B, N, C = x.shape
        x = x.transpose(1, 2).view(B, C, H, W)
        x1,x2,x3,x4 = torch.split(x,C//4,dim=1)
        x1_3 = self.dwconv3(x1)
        x2_3 = self.dwconv3(x2 + x1_3)
        x3_3 = self.dwconv3(x3 + x2_3)
        x4_3 = self.dwconv3(x4 + x3_3)
        x1_c   = torch.cat((x1_3,x2_3,x3_3,x4_3),1) + self.dwconv3_1(x)
        x2_c   = self.dwconv3_1(x)
        x1_c = x1_c.flatten(2).transpose(1, 2)
        x2_c = x2_c.flatten(2).transpose(1, 2)
        return x1_c,x2_c
...
class FFT_fusion(nn.Module): # Fourier-space merging strategy (FMS)
    ...
    def forward(self, x_T, x_C, H, W):
        B, N, C = x_T.shape
        x_T = x_T.transpose(1, 2).view(B, C, H, W)
        x_C = x_C.transpose(1, 2).view(B, C, H, W)
...
@register_model
def glt_b2(pretrained=False, **kwargs): # DPU-Former Encoder
    model = GLformer(
        patch_size=4, embed_dims=[64, 128, 320, 512], num_heads=[1, 2, 5, 8], mlp_ratios=[8, 8, 4, 4], qkv_bias=True,
        norm_layer=partial(nn.LayerNorm, eps=1e-6), depths=[3, 4, 10, 3], sr_ratios=[8, 4, 2, 1], **kwargs)
